...
string cube_to_facelets(const CubeState& cube) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        cout << "Position " << i << ", piece " << piece << ", ori " << ori << ":" << endl;
        
        for (int j = 0; j < 3; j++) {
            int faceletIdx = cornerFacelet[i][j];
            int colorIdx = (j - ori + 3) % 3;
            char color = cornerColor[piece][colorIdx];
            f[faceletIdx] = color;
            cout << "  Facelet " << faceletIdx << " = " << color 
                 << " (piece color[" << colorIdx << "])" << endl;
        }
    }
...
int main() {
    ...
    cout << endl << "Result: " << facelets << endl;
